...
fn test_not_in_string_values() {
    let result =
        translate("SELECT * FROM products WHERE status NOT IN ('deleted', 'archived')").unwrap();
...
fn test_in_combined_with_other_conditions() {
    let result = translate(
        "SELECT * FROM products WHERE category IN ('electronics', 'accessories') AND price > 100",
    )
    .unwrap();
    assert_eq!(
        result.query_string,
...
fn test_vector_knn_query_string() {
    ...
    use sql_parser::ast::{DistanceMetric, SelectQuery, VectorSearch};
    use sql_parser::translator;
...
fn test_hybrid_vector_search() {
    ...
    use sql_parser::ast::{Condition, DistanceMetric, SelectQuery, Value, VectorSearch};
    use sql_parser::translator;
...
fn test_ft_hybrid_command() {
    ...
    use sql_parser::ast::{DistanceMetric, HybridSearch, SelectQuery, VectorSearch};
    use sql_parser::translator;
...
fn test_vector_search_sql_with_fields() {
    ...
    let result =
        translate("SELECT name, price FROM products ORDER BY embedding <-> '[0.1, 0.2]' LIMIT 5")
            .unwrap();
...
fn test_vector_search_parser_recognizes_only_supported_pgvector_operators() {
    use sql_parser::ast::DistanceMetric;
...
fn test_not_combined_with_and() {
    let result =
        translate("SELECT * FROM idx WHERE NOT (category = 'electronics') AND price > 50").unwrap();
    assert_eq!(
        result.query_string,
...
fn test_count_distinct_with_group_by() {
    let result =
        translate("SELECT region, COUNT_DISTINCT(user_id) FROM events GROUP BY region").unwrap();
    assert_eq!(result.command, Command::Aggregate);
    let args = &result.arguments;
...
fn test_is_not_null_requires_index_missing_with_schema() {
    ...
    let err =
        translate_with_schema("SELECT * FROM idx WHERE category IS NOT NULL", &schema).unwrap_err();
