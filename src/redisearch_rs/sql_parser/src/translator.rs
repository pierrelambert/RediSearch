...
use crate::{Command, QuerySchema, Translation};

/// Translates a parsed SQL query into a RQL translation.
pub fn translate(query: SelectQuery) -> Result<Translation, SqlError> {
    translate_with_schema(query, &QuerySchema::default())
}

/// Translates a parsed SQL query into a RQL translation using schema metadata.
pub fn translate_with_schema(
    ...
    let query_string = build_query_string(&query, schema)?;
...
fn build_query_string(query: &SelectQuery, schema: &QuerySchema) -> Result<String, SqlError> {
    // For FT.HYBRID, the query string is just the text query (no KNN syntax in query)
    if query.hybrid_search.is_some() {
        return build_hybrid_query_string(query, schema);
    }
    ...
    let base_query = if query.conditions.is_empty() {
        ...
        let parts: Vec<String> = query
            .conditions
            .iter()
            .map(|condition| translate_condition(condition, schema))
...
fn validate_string_exact_match(field: &str, schema: &QuerySchema) -> Result<(), SqlError> {
    ...
    if capabilities.supports_text_queries && !capabilities.supports_tag_queries {
...
fn translate_condition(condition: &Condition, schema: &QuerySchema) -> Result<String, SqlError> {
    match condition {
        Condition::Equals { field, value } => match value {
            // String equality uses TAG syntax for exact match
            Value::String(s) => {
                validate_string_exact_match(field, schema)?;
                Ok(format!("@{field}:{{{}}}", escape_tag_value(s)))
            }
        ...
        Condition::In {
            ...
            if all_strings {
                ...
                let values_str = values
                    .iter()
                    .map(|v| escape_tag_value(&v.to_rql_string()))
                    .collect::<Vec<_>>()
                    .join("|");
        ...
        Condition::Not(inner) => {
            ...
            let inner_str = translate_condition(inner, schema)?;
...
mod tests {
    ...
    use crate::{FieldCapabilities, QuerySchema};
    ...
    #[test]
    fn test_translate_string_equality_escapes_tag_syntax() {
        let query = SelectQuery::new("idx").with_condition(Condition::Equals {
            field: "status".to_string(),
            value: Value::String(r"a{b}|c\\d}".to_string()),
        });

        let result = translate(query).unwrap();

        assert_eq!(result.query_string, r"@status:{a\{b\}\|c\\\\d\}}")
    }
    ...
    #[test]
    fn test_translate_in_strings_escapes_each_value() {
        let query = SelectQuery::new("idx").with_condition(Condition::In {
            field: "category".to_string(),
            values: vec![
                Value::String("brace}".to_string()),
                Value::String(r"pipe|slash\\".to_string()),
            ],
            negated: false,
        });
        ...
        assert_eq!(result.query_string, r"@category:{brace\}|pipe\|slash\\\\}");
    }
    ...
    #[test]
    fn test_translate_with_schema_rejects_text_equality() {
        let schema = QuerySchema::new(9).with_field("title", FieldCapabilities::text());
        let query = SelectQuery::new("idx").with_condition(Condition::Equals {
            field: "title".to_string(),
            value: Value::String("redis".to_string()),
        });

        let err = translate_with_schema(query, &schema).unwrap_err();
