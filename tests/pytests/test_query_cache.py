...
class TestQueryCache(ModuleTestCase):
    """
    ...
    def test_cache_hit_identical_query(self):
        ...
        self.assertIn('query_cache_stats', info1)
        cache_stats1 = info1['query_cache_stats']
        self.assertEqual(cache_stats1['misses'], 1)
        self.assertEqual(cache_stats1['hits'], 0)
        ...
        # Verify cache hit
        self.assertEqual(cache_stats2['hits'], 1)
        self.assertEqual(cache_stats2['misses'], 1)
    ...
    def test_cache_miss_different_query(self):
        ...
        info1 = index_info(env, 'idx')
        self.assertEqual(info1['query_cache_stats']['misses'], 1)
        ...
        self.assertEqual(info2['query_cache_stats']['misses'], 2)
        self.assertEqual(info2['query_cache_stats']['hits'], 0)
    ...
    def test_cache_invalidation_on_add(self):
        ...
        self.assertEqual(res1[0], 1)  # 1 result
        
        # Second query - cache hit
        env.cmd('FT.SEARCH', 'idx', 'hello')
        info = index_info(env, 'idx')
        self.assertEqual(info['query_cache_stats']['hits'], 1)
        ...
        self.assertEqual(res3[0], 2)  # 2 results now
        info3 = index_info(env, 'idx')
        
        # Should have 2 misses (initial + after add)
        self.assertEqual(info3['query_cache_stats']['misses'], 2)
        self.assertEqual(info3['query_cache_stats']['hits'], 1)
    ...
    def test_cache_invalidation_on_update(self):
        ...
        self.assertEqual(info['query_cache_stats']['misses'], 2)
        self.assertEqual(info['query_cache_stats']['hits'], 1)
    ...
    def test_cursor_queries_not_cached(self):
        ...
        if 'query_cache_stats' in info:
            ...
            self.assertEqual(info['query_cache_stats']['hits'], 0)

    def test_unlimited_queries_not_cached(self):
    ...
    def test_different_limit_different_cache_key(self):
    ...
    def test_different_offset_different_cache_key(self):
    ...
    def test_cache_stats_verification(self):
        ...
        self.assertIn('memory_bytes', cache_stats)
        self.assertIn('evictions', cache_stats)
        ...
        self.assertEqual(cache_stats['entries'], 2)

        # Memory should be > 0
        self.assertGreater(cache_stats['memory_bytes'], 0)

        # No evictions yet
        self.assertEqual(cache_stats['evictions'], 0)
    ...
    def test_cache_multiple_indexes(self):
        ...
        cache_stats1 = info1['query_cache_stats']
        self.assertEqual(cache_stats1['hits'], 1)
        self.assertEqual(cache_stats1['misses'], 1)
