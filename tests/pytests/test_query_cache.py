...
def test_cache_hit_identical_query(env):
    ...
    env.assertIn('global_query_cache_stats', info1)
    cache_stats1 = info1['global_query_cache_stats']
    ...
    cache_stats2 = info2['global_query_cache_stats']
...
def test_cache_replays_scores_and_fields(env):
    """Test that cached FT.SEARCH replies preserve scores and returned fields"""
    conn = getConnectionByEnv(env)

    env.cmd('FT.CREATE', 'idx', 'SCHEMA', 'title', 'TEXT', 'year', 'NUMERIC', 'SORTABLE')
    conn.execute_command('HSET', 'doc1', 'title', 'hello world', 'year', '2020')
    conn.execute_command('HSET', 'doc2', 'title', 'hello redis', 'year', '2021')

    query = ['FT.SEARCH', 'idx', 'hello', 'WITHSCORES', 'RETURN', '2', 'title', 'year',
             'SORTBY', 'year', 'DESC', 'LIMIT', '0', '10']
    res1 = env.cmd(*query)
    res2 = env.cmd(*query)

    cache_stats = index_info(env, 'idx')['global_query_cache_stats']
...
def test_cache_miss_different_query(env):
    ...
    info1 = index_info(env, 'idx')
    env.assertEqual(info1['global_query_cache_stats']['misses'], 1)
    ...
    env.assertEqual(info2['global_query_cache_stats']['misses'], 2)
    env.assertEqual(info2['global_query_cache_stats']['hits'], 0)
...
def test_cache_invalidation_on_add(env):
    ...
    env.assertEqual(info['global_query_cache_stats']['hits'], 1)
    ...
    env.assertEqual(info3['global_query_cache_stats']['misses'], 2)
    env.assertEqual(info3['global_query_cache_stats']['hits'], 1)
...
def test_cache_invalidation_on_update(env):
    ...
    env.assertEqual(info['global_query_cache_stats']['misses'], 2)
    env.assertEqual(info['global_query_cache_stats']['hits'], 1)
...
def test_cache_disabled_when_size_zero(env):
    ...
    info = index_info(env, 'idx')
    env.assertFalse('global_query_cache_stats' in info)
...
def test_cursor_queries_not_cached(env):
    ...
    info = index_info(env, 'idx')
    if 'global_query_cache_stats' in info:
        # If stats exist, hits should be 0
        env.assertEqual(info['global_query_cache_stats']['hits'], 0)
...
def test_different_limit_different_cache_key(env):
    ...
    info = index_info(env, 'idx')
    cache_stats = info['global_query_cache_stats']
...
def test_different_return_order_different_cache_key(env):
    """Test that RETURN field order participates in the cache key"""
    conn = getConnectionByEnv(env)

    env.cmd('FT.CREATE', 'idx', 'SCHEMA', 'title', 'TEXT', 'year', 'NUMERIC')
    conn.execute_command('HSET', 'doc1', 'title', 'hello world', 'year', '2020')

    env.cmd('FT.SEARCH', 'idx', 'hello', 'RETURN', '2', 'title', 'year')
    env.cmd('FT.SEARCH', 'idx', 'hello', 'RETURN', '2', 'title', 'year')
    env.cmd('FT.SEARCH', 'idx', 'hello', 'RETURN', '2', 'year', 'title')

    cache_stats = index_info(env, 'idx')['global_query_cache_stats']
    env.assertEqual(cache_stats['hits'], 1)
    env.assertEqual(cache_stats['misses'], 2)

def test_different_scorer_different_cache_key(env):
    """Test that SCORER participates in the cache key"""
    conn = getConnectionByEnv(env)

    env.cmd('FT.CREATE', 'idx', 'SCHEMA', 'title', 'TEXT')
    conn.execute_command('HSET', 'doc1', 'title', 'hello world')
    conn.execute_command('HSET', 'doc2', 'title', 'hello redis world')

    env.cmd('FT.SEARCH', 'idx', 'hello', 'SCORER', 'BM25STD', 'WITHSCORES')
    env.cmd('FT.SEARCH', 'idx', 'hello', 'SCORER', 'BM25STD', 'WITHSCORES')
    env.cmd('FT.SEARCH', 'idx', 'hello', 'SCORER', 'TFIDF', 'WITHSCORES')
...
def test_cache_stats_verification(env):
    ...
    env.assertAlmostEqual(float(cache_stats['hit_rate']), expected_hit_rate, delta=0.01)
...
def test_cache_hit_rate_calculation(env):
    ...
    env.assertEqual(cache_stats['misses'], 1)
    env.assertAlmostEqual(float(cache_stats['hit_rate']), 0.9, delta=0.01)

def test_cache_with_aggregate_query(env):
    """Test that FT.AGGREGATE queries are not cached"""
    env = env
    conn = getConnectionByEnv(env)
    ...
    env.assertEqual(cache_stats['hits'], 0)
    env.assertEqual(cache_stats['misses'], 0)
...
def test_cache_multiple_indexes(env):
    ...
    # Cache stats are global even though cache entries are index-specific.
    # Both FT.INFO calls should therefore report aggregate counters.
    # Check stats for idx1
    info1 = index_info(env, 'idx1')
    cache_stats1 = info1['global_query_cache_stats']
    env.assertEqual(cache_stats1['hits'], 2)
    env.assertEqual(cache_stats1['misses'], 2)

    # Check stats for idx2
    info2 = index_info(env, 'idx2')
    cache_stats2 = info2['global_query_cache_stats']
    env.assertEqual(cache_stats2['hits'], 2)
    env.assertEqual(cache_stats2['misses'], 2)
