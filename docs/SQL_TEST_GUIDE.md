...
Manual guide for FT.SQL design-time and exploratory testing. The current audited
delivery is the hardened experimental/beta subset gated by `SQL_ENABLED`; not
every advanced example below is part of the verified shipping scope.
...
The scenarios below reflect the current audited FT.SQL subset.
...
The following test scenarios describe planned functionality that has not been
implemented or audited. They are retained for planning purposes.
